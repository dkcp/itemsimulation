...
router.post('/characters', authMiddleware, async (req, res) => {
    try {
        ...
        const newCharacter = await prisma.characters.create({
            data: { accountId: accountId, name },
        });
        if (!newCharacter)
            return res
                .status(404)
                .json({ message: '캐릭터 생성에 실패했습니다.' });
        else return res.status(201).json({ message: "캐릭터 생성 완료" ,data: newCharacter.characterId });
    } catch (error) {
