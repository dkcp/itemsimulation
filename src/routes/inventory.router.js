...
// 아이템 구매
router.patch(
    '/inventory/purchase/:characterId',
    authMiddleware,
    async (req, res, next) => {
        try {
            ...
            const result = await prisma.$transaction(
                async (tx) => {
                    ...
                    if (!existingInventory) {
                        ...
                        const updatedInventory = await tx.inventory.update({
                            data: {
                                characterId: +characterId,
                                item_code,
                                count: existingInventory.count + count,
                            },
                            where: {
                    ...
                    return {
                        data: `구매 후 게임 머니 잔액 : ${character.money} -> ${updatedCharacter.money}`,
                    };
                },
                {
                    isolationLevel:
    ...
    async (req, res, next) => {
        try {
            ...
            if (!existingInventory)
                return res
                    .status(404)
                    .json({ message: '캐릭터가 보유하지 않은 아이템입니다.' });

            const result = await prisma.$transaction(
                async (tx) => {
                    if (existingInventory.count - count >= 0) {
                        // 판매 후 개수가 0이면 inventory에서 삭제
                        if (existingInventory.count - count === 0) {
                        ...
                        return {
                            data: `판매 후 게임 머니 잔액 : ${character.money} -> ${updatedCharacter.money}`,
                        };
                    } else
                        throw new Error(
                            '보유중인 수량보다 많이 팔 수 없습니다.'
                        );
                },
                {
                    isolationLevel:
