...
router.post('/accounts/sign-up', async (req, res, next) => {
    try {
        ...
        //아이디 중복 검사
        const existingId = await prisma.accounts.findFirst({ where: { accountId: accountId } });
        if (existingId) return res.status(409).json({ message: '이미 존재하는 아이디입니다.' });

        //비밀번호 유효성 검사 : 최소 6자 이상 / 비밀번호 확인과 일치
        if (password.length < 6) return res.status(404).json({ message: '비밀번호는 6자 이상이어야 합니다.' });
        if (password !== password_chk) {
            return res.status(404).json({ message: '비밀번호 확인과 일치하지 않습니다.' });
        }
        const hashedPassword = await bcrypt.hash(password, 10);

        //계정 생성
        const result = await prisma.$transaction(async tx => {
            const newAccount = await tx.accounts.create({ data: { accountId, password: hashedPassword, name} });
            //롤백할 필요는 없지만...
...
router.post('/accounts/sign-in', async (req, res, next) => {
    ...
    const token = jwt.sign({ accountId: account.accountId }, process.env.JWT_KEY);
...
router.delete('/accounts/:accountId', async (req, res, next) => {
    ...
    const account = await prisma.accounts.findFirst({ where: { accountId: accountId } });
